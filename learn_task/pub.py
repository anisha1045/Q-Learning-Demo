...
class Color_Detection_Pub:
   ...
   def color_detection_publisher(self):
       ...
       while not rospy.is_shutdown():
           ...
           x1 = width // 3 #Vertical Lines
           x2 = 2 * width // 3 #Vertical Lines
           y1 = height // 3    #Horizontal Lines
           y2 = 2 * height //3 #Horizontal Lines
           ...
           for countour in contours:
               ...
               pos_of_grid = f"Grid Position: ({col}, {row})"  #Acquires where the detected object is in the grid
           ...
           cv2.imshow('Window', frame) #Contour Detection, only the color
...
if __name__ == '__main__':
    try:
        pub_object = Color_Detection_Pub()
